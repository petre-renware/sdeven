...
* tbd... to integrate old version (`v6.0` in example) in current (new future) version:
    ...
    3. create `doc_src/v6.0/` directory with an empty `index.md` text file (just to exists when used as reference, if feel better can write a comment inside it as in it will never be generated as html) 
